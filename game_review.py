...
def get_review(game_url):
    ...
    while True:
        ...
        for single_review in review_list:
            ...
            print(review_data_posted)
            if '年' in review_data_posted:
                ...
                review_data_posted = re.search('(\d+)月(\d+)日', review_data_posted)
                review_data_posted = '-'.join([review_data_posted[i] for i in range(1, 3)])
            # 评论文字
